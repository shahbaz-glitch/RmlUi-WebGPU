...
static WGPUTextureView CreateView(
    WGPUTexture texture,
...
static void MatrixToArray(
    const Rml::Matrix4f& matrix,
    float* out)
{
    std::memcpy(
        out,
    ...
    RenderInterface_WebGPU* renderer)
{
    ...
    return renderer->CompileGeometry(
        mesh.vertices,
        mesh.indices);
}

static Rml::CompiledGeometryHandle CreateFullscreenUVGeometry(
    RenderInterface_WebGPU* renderer,
    float u0,
    float v0,
    float u1,
    float v1)
{
    ...
    {
        const float u = vertex.tex_coord.x;
        const float v = 1.0f - vertex.tex_coord.y;
...
static void DestroyTextureResource(
    Gfx::WebGPUTexture& texture)
{
    ...
    WGPUSampler sampler)
{
    ...
    if (!device ||
        !texture_bgl ||
        !sampler ||
        color_format == WGPUTextureFormat_Undefined ||
    ...
    if (!m_fb_layers[0].color_texture ||
        !m_fb_layers[0].color_view)
    {
        Shutdown();
        return;
    }
...
void RenderInterface_WebGPU::SetViewport(
    int width,
    int height)
{
    if (width <= 0 || height <= 0)
...
void RenderInterface_WebGPU::BeginFrame(
...
void RenderInterface_WebGPU::CreateBindGroupLayouts()
{
    ...
    transform.buffer.type =
        WGPUBufferBindingType_Uniform;
    transform.buffer.hasDynamicOffset = true;
    transform.buffer.minBindingSize =
        sizeof(TransformUniform);
    ...
    texture_entries[0].visibility =
        WGPUShaderStage_Fragment;
    texture_entries[0].sampler.type =
        WGPUSamplerBindingType_Filtering;

    texture_entries[1].binding = 1;
    texture_entries[1].visibility =
        WGPUShaderStage_Fragment;
    texture_entries[1].texture.sampleType =
...
void RenderInterface_WebGPU::CreatePipelines()
{
    ...
    filter_layout_desc.bindGroupLayouts =
        filter_groups;
    ...
    texture_layout_desc.bindGroupLayouts =
        texture_groups;
    ...
    mask_layout_desc.bindGroupLayouts =
        mask_groups;
    ...
    transform_layout_desc.bindGroupLayouts =
        transform_groups;
    ...
    Rml::Vector2f translation)
{
    ...
    case Rml::ClipMaskOperation::SetInverse:
    {
        ...
        /*
         * Inverse mask semantics:
         *
         *   untouched stencil = 1
         *   supplied geometry = 0
         *   final draw tests Equal(1)
         */
        m_stencil_ref_value = 0;
        m_stencil_equal = false;
...
{
    if (!m_command_encoder ||
        !source_destination.color_texture ||
        !temp.color_texture ||
        sigma <= 0.0f)
    {
        return;
    ...
    if (tertiary.texture_bind_group)
    {
        wgpuRenderPassEncoderSetBindGroup(
            m_render_pass,
            1,
    ...
    Rml::Span<const Rml::CompiledFilterHandle> filter_handles)
{
    ...
    const Rml::Rectanglei saved_scissor =
        m_scissor_state;
    ...
    for (auto filter_handle : filter_handles)
    {
        ...
        {
            /*
             * The shader contract only needs:
             *
             *   color
             *   texture coordinate clamp
             *
             * Offset is applied in the fullscreen UVs here.
             * This keeps the CPU uniform layout identical to
             * DropShadowUniformData and avoids the old offset/sigma
             * structure mismatch.
             */
            ...
            /*
             * Composite original content over the shadow.
             */
            UsePipeline(
                WebGPUPipelineId::Passthrough);
...
RenderInterface_WebGPU::SaveLayerAsTexture()
{
    ...
    /*
     * Render directly into the saved texture.
     *
     * The source region is mapped into [0,1].
     * The explicit Y inversion matches the top-left RmlUi window
     * convention without introducing a second CPU-side texture flip.
     */
    EndCurrentRenderPass();
        ...
        geometry->index_buffer)
    {
        ...
        if (save_geometry)
        {
            ...
            if (save_gpu &&
                save_gpu->vertex_buffer &&
                save_gpu->index_buffer)
            {
                wgpuRenderPassEncoderSetVertexBuffer(
                    m_render_pass,
                    0,
    ...
    const Rml::Dictionary& parameters)
{
    ...
    if (name == "opacity")
    {
        ...
        /*
         * Premultiplied RGBA:
         * scaling all four channels is equivalent to opacity.
         */
        filter->color_matrix =
            Rml::Matrix4f::Diag(
                value,
    ...
    const Rml::Dictionary& parameters)
{
        ...
        [&](const Rml::Dictionary& shader_parameters)
    {
        ...
        shader->stop_positions.resize(
            count);

        shader->stop_colors.resize(
            count);
    ...
    else if (name == "radial-gradient")
    {
        ...
        shader->v = {
            1.0f /
                std::max(radius.x, 0.0001f),
            1.0f /
                std::max(radius.y, 0.0001f)
        };
